...
//Segment Tree can be build for any associative operation a x (b x c) == (a x b) x c where x is associative operation
	...
	void build(vector<int> &arr,int x,int lx,int rx){
		...
		if(rx-lx==1){
			// since we have sums array of extra size to make to nearst power of 2
...
// set function set(i,v) which is to set arr[i]=v  
	void set(int i,int v,int x,int lx,int rx){
		...
		//after recursion we have to recalculate sum at node x to its left child and right child 
		sums[x]=min(sums[2*x+1],sums[2*x+2]);
	}
	...
	int sum(int l,int r,int x,int lx,int rx){
		...
		// case-2 when segment [lx--rx] completly inside of segment[l---r] return segment sum 
		else if(l<=lx && rx<=r) return sums[x];
		//else go to left child and right child to find segment and return sum 
		int m=(lx+rx)/2;
		int left=sum(l,r,2*x+1,lx,m);
		int right=sum(l,r,2*x+2,m,rx);
		return min(left,right);
	}
	// sum for a segment from [l----r]
	int sum(int l,int r){
